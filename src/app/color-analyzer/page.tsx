...
export default function ColorAnalyzerPage() {
  ...
  const autoApply = (extracted: string[]) => {
    if (extracted.length >= 3) applyTheme({ primary: extracted[0], secondary: extracted[1], accent: extracted[2] });
    else if (extracted.length === 2) applyTheme({ primary: extracted[0], secondary: extracted[1], accent: extracted[1] });
    else if (extracted.length === 1) applyTheme({ primary: extracted[0], secondary: extracted[0], accent: "#18181b" });
  };
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-[var(--color-accent)]/50">上传一张图片，自动提取主色调并按 60-30-10 比例应用到全站主题。</p>
      <div className="mt-8 grid gap-8 sm:grid-cols-2">
        <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-[var(--color-secondary)]/40 bg-[var(--color-primary)]/10 p-8 text-center min-h-[200px]">
        ...
        <div>
          <h2 className="font-semibold mb-3 text-[var(--color-accent)]">提取的色调</h2>
          {colors.length > 0 ? (
            <div className="space-y-3">
              {colors.map((hex, i) => (
                <div key={hex} className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-lg border border-[var(--color-secondary)]" style={{ backgroundColor: hex }} />
                  <div><code className="text-sm font-mono text-[var(--color-accent)]">{hex}</code><span className="ml-2 text-xs text-[var(--color-accent)]/50">{i===0?"60% 主色":i===1?"30% 辅色":"10% 强调色"}</span></div>
                </div>
              ))}
              <div className="flex gap-3 pt-4">
                <button onClick={() => autoApply(colors)} className="rounded-lg bg-[var(--color-accent)] px-4 py-2 text-sm font-medium text-[var(--color-primary)] transition hover:opacity-80">应用主题</button>
